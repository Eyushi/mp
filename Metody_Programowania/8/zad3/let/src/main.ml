...
let rename_expr e =
  let rec rename a (bound : dc3) (s : string) =
    match  e with
    | Int x -> Int x
    |Bool y -> Bool y
    | Var v -> if M.mem v bound then Var (M.find v bound) else Var v
    |Binop(op, e1, e2) -> Binop(rename e1 bound (s^ "l".rename e2 bound (s^"R")))
    |If(e1, e2, e3) -> If()

    |Let(x, e1, e2)
    Let(s, rename e1 bound (s^"l").rename e2 (M.add x s bound) (s ^ "r"))
...
let rec rename_expr (e : expr) : expr =
  match e with
  | Var _ -> Var (fresh_var ())
  | Int _ | Bool _ as const -> const
  | Binop (op, e1, e2) -> Binop (op, rename_expr e1, rename_expr e2)
  | If (p, t, e) -> If (rename_expr p, rename_expr t, rename_expr e)
  | Let (_, e1, e2) -> Let (fresh_var (), rename_expr e1, rename_expr e2) *)

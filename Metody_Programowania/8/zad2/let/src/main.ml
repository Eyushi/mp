...
(* 1 -- zacząc od próby zrobienia 
   if e1 e2 e3

    funkcja -- robi listę wszystkich podwyrażeń dla e1 e2 e3
    dla każdej pary sprawdzamy czy są alfa-równoważne np jest e4
    (alfa nam gwarantuje brak problemów)
    let y = e4 in If(subst1, subst2, subst3)

    też skorzystać z zadania 3 -- ponazywać zmienne np z # (najlepiej? bo nie istnieje w składni)
     
    
    
    wszystkie podwyrażenia:
    podwyrażenia + rek podwyrażenia
    nie zwracać bool i int (nietrywialne)
    let x = 2 + 2 in x + 1
    2 + 2   2   2
    x + 1   x   1
    ...
    albo najpierw -- każda zmienna związana musi być inna i potem sprawdziź alfa równoważność ??
    nie wystarcza bo np prechwycenie zmiennych
    *)